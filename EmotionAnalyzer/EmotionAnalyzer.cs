...
namespace SentEmo
{
    public class SentEmoAnalyzer
    {
        ...
        public EmotionState DoAnalysis(string TextData)
        {
            //count speech parts 
            string[] tokens = Library.tokenizer.Tokenize(TextData); //divide sentence into words
            ...
            EmotionState result = analyse_sentence(tokens, tags); //merge each result for each sentence to get final result
        ...
        private EmotionState analyse_sentence(string[] words, string[] parts)
        {
            ...
            {
                double coeff = word == HelperMethods.low_registry(word) ? 1 : high_capital_factor;
            ...
            double syntax_value = MathAnalyzer.syntax_value(parts); //syntax value is a little point here
            //but improtant its range is from 0 to 0.1 

            Result.Sentiment =MathAnalyzer. calculate_sentiment(Result, pos_val, neg_val, incl_val, excl_val, syntax_value); //calculating sentiment

            Result = MathAnalyzer.calculate_emotions(Result, syntax_value); //calculating emotions
            //it is improtant that before running you already have calculatid sentiment

            Result.Inclusion = MathAnalyzer.calculate_inclusion(Result, incl_val, excl_val, syntax_value);
